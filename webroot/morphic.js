...
HandMorph.prototype.processMouseMove = function (event) {
    ...
    if (sqSendEvent && event.isTrusted) {sqSendEvent(event);}
    this.setPosition(pos);
...
HandMorph.prototype.drawNew = function() {
    if (this.id) {
        var context;
        this.image = newCanvas(this.extent());
        context = this.image.getContext('2d');
...
WorldMorph.prototype.fullDrawOn = function (aCanvas, aRect) {
    ...
    for (var k in this.hands) {
       this.hands[k].fullDrawOn(aCanvas, aRect);
    }
};
...
WorldMorph.prototype.addHandFor = function(idString) {
    var hand = new HandMorph(this);
    hand.id = idString;
    hand.setExtent(new Point(27, 43));
    hand.drawNew();
    if (this.hands[idString]) {
      this.removeHandAt(idString);
    }
    this.hands[idString] = hand;
    hand.id = idString;
    return hand;
};

WorldMorph.prototype.removeHandAt = function(id) {
...
function receiveRemoteEvent(idString, buf) {
    ...
    if (hand) {
        ...
        switch (type) {
            case "mousemove":
                hand.processMouseMove(evt);
                break;
            case "mousedown":
                hand.processMouseDown(evt);
                break;
            case "mouseup":
